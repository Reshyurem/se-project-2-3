...
function Course(props) {
        ...
        const checkCourse = async () => {
            try {
                ...
                if (
                    ...
                    userData.subscribedCourses.includes(props.courseId)
                ) {
                    setHasCourse(true);
                } else {
    ...
    useEffect(() => {
        const checkCourseAlert = async () => {
            try {
                ...
                const userData = userDoc.data();
                if (
                    userData.courseAlerts &&
                    userData.courseAlerts.includes(props.courseId)
                ) {
                    setIsAlertSet(true);
                } else {
                    setIsAlertSet(false);
    ...
    const handleSubmitReview = async (e) => {
        ...
        try {
            ...
            const courseQuery = await firebase
                .firestore()
                .collection("courses")
                .where("courseId", "==", props.courseId)
                .limit(1)
        ...
        let courseId = String(props.courseId);
        let message = String(newReview);
    ...
    return (
        <>
            ...
            <div>View Chatroom {hasCourse}</div>
            {hasCourse && (
                <div className="course-container">
